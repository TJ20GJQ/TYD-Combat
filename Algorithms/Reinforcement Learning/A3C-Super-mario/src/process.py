...
def local_train(index, opt, global_model, optimizer, save=False):
    ...
    while True:
        ...
        for _ in range(opt.num_local_steps):
            ...
            entropy = -(policy * log_policy).sum(1, keepdim=True)  # 计算当前熵值，保持多样性
        ...
        for value, log_policy, reward, entropy in list(zip(values, log_policies, rewards, entropies))[::-1]:
            ...
            next_value = value
            actor_loss = actor_loss + log_policy * gae  # 越有优势，出现概率越大
            R = R * opt.gamma + reward
            critic_loss = critic_loss + (R - value) ** 2 / 2  # 预期与标签越接近越好
            entropy_loss = entropy_loss + entropy  # 熵越大越好，保持多样性
